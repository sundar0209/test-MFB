...
const MonthlyFinance = () => {
    ...
    const [closecnfrmShow, setClosecnfrmShow] = useState(false);
    console.log("closecnfrmShow",closecnfrmShow);
    ...
    const handleShow5 = (e) => {
        closeLoan()
        //  setClosecnfrmShow(true);
        // setLoanClose(false)
    }

    const LoanhandleClose2 = () => { setLoanClose(false) }
    const LoanhandleShow = () => {
        setLoanClose(true);
        setShow(false)
    }
    ...
    const [principalcloaseAmount, setPrincipalcloseamount] = useState("");
    const [interestcloaseAmount, setInterestcloseamount] = useState("");
    ...
    const [closableLables, setClosableLables] = useState();
    ...
    // const enableDate = (checkId, editId, inputId, amountId, penaltyId, comments) => {
    ...
    const loanFilter = (data) => {
        ...
        axiosObject.post("monthlyFilter/condition", request).then(response => {
    ...
    const dailyloanFilter = (data) => {
        ...
        axiosObject.post("monthlyloanTypeFilter/condition", request).then(response => {
    ...
    const pclear = () => {
        ...
        setInterestAmount("")
        setEditloanLables([]);
        //console.log("pclear", pclear);



    }
    // const loanTypeFilter = (data) => {
    ...
    const agentFilter = (data) => {
        ...
        axiosObject.post("monthlySalesFilter/condition", request).then(response => {
            ...
            axiosObject.post("custloanApprove/update", request)
                .then((response) => {
                    ...
                    if (response.data.success == true) {
                        ...
                        setPopupActionPath("/monthlyfinance")
    ...
    const SearchInventory = () => {
        ...
        axiosObject.post("monthlyloanTypeSearch/condition", request).then(response => {
    ...
    const getCustData = () => {
        ...
        axiosObject
            .post("monthlyFinance/condition", request)
    ...
    const detailsView = (data) => {
        ...
        axiosObject.post("loandetailsview/condition", request)
            ...
            .then((response) => {
                ...
                setLoanId(response.data.data[0]?.loan_id);
                console.log("loanid==>",response.data.data[0].loan_id);
                setLoanViewId(response.data.data[0]?.vendorloan_id);
                ...
                setDate(response.data.data[0]?.installment_date);
                setAmount(response.data.data[0]?.installment_amount);
                setPrincipalcloseamount(response.data.data[0]?.loan_close_amount);
                console.log("closeloamamt", response.data.data[0]?.loan_close_amount);
                setInterestcloseamount(response.data.data[0]?.loan_close_intrest_amount);
            ...
            axiosObject.post("loandetailEdit/update", request)
                .then((response) => {
                    ...
                    if (response.data.success == true) {
                        setCnfrmShow(false);
                        togglePopup()
                        setPopupTitle("Monthly Finance");
                        setPopupMsg("Monthly Finance Updated Successfully");
                        setPopupType("success");
                        setPopupActionType("refresh");
                        setPopupActionValue("ok");
    ...
    const closeLoan = (e) => {
        ...
        console.log("principalcloaseAmount out");
        ...
        if (!principalcloaseAmount) {
            console.log("principalcloaseAmount");
            return;
        }
        if (!interestcloaseAmount) {
            console.log("interestcloaseAmount");
            return;
        }
        ...
        let request = {
            ...
            token: loginDetails[0].token,
            vendor_id: loginDetails[0].vendor_id,
            loan_close_amount: principalcloaseAmount,
            loan_close_intrest_amount: interestcloaseAmount

        }
        console.log("==req==", request);

        {
            axiosObject.post("loanclosed/update", request)
                .then((response) => {
                    console.log("resresponse.data.data", response.data.data)
                    if (response.data.success == true) {
                        setLoanClose(false);
                        togglePopup()
                        setPopupTitle("Loan Closed");
                        setPopupMsg("Monthly Loan Closed Successfully");
                        setPopupType("success");
                        setPopupActionType("refresh");
                        setPopupActionValue("ok");
                        // setPopupActionPath("/customers")
    ...
    const columns = [
...

        {
            ...
            cell: (row) => {
                return (
                    <div>
                        <a onClick={() => handleShowview3(row.loan_id)}>{row.loan_id}</a>
    ...
    console.log("newMonthly calling", newMonthly);
    ...
    // const error = !principalcloaseAmount && (submitClicked) ? 'error' : '' && !interestcloaseAmount && (submitClicked) ? 'error' : ''
    ...
    // const getCloseLoanDeatils = (value, label) => {
    //     // console.log("label calling", label);

    //     if (label === "Principal Amount") {
    //        setPrincipalcloseamount(value);
    //        if (closableLables.length > 0) {
    //           if (closableLables.filter((data) => data.label === label).length === 0) {
    //              // console.log("if calling");
    //              setClosableLables([...closableLables, { label }]);
    //           }
    //        }
    //        else {
    ...
    //        }
    //     }
    //     if (label === "Interest closeAmount") {
    //        setInterestcloseamount(value);
    //        if (closableLables.length > 0) {
    //           if (closableLables.filter((data) => data.label === label).length === 0) {
    //              // console.log("if calling");
    //              setClosableLables([...closableLables, { label }]);
    ...
    const getEditLoanDeatils = (value, label) => {
        ...
        if (label === "Enable SMS Notification") {
            //const item = value.replace(/\D/g, "");
            // console.log("item calling", item);
            //setNumber(true);
            setEnablesms(value);
            if ("Enable SMS Notification" !== "") {
                // console.log("if calling");
                if (editloanLables.length > 0) {
                    if (editloanLables.filter((data) => data.label === label).length === 0) {
                        setEditloanLables([...editloanLables, { label }]);
        ...
        if (label === "monthly") {
            ...
            console.log("monthly Loan============", value);
    ...
    return (
        <div>
                ...
                <section id="content-wrapper">
                    <div className="row">
                        <div className="col-lg-4 col-md-12 col-sm-12 col-12">
                            <h1 className='pageHead mt-2'>Monthly Finance </h1>
                        </div>
            ...
            {/* <button className="btn btn-primary btn-outline smallBtn"  onClick={handleShow}>
       View Loan
      </button> */}
                    ...
                    <>
                        <Modal.Header closeButton>
                            ...
                            <div className='loanAmt'>
                                <div className='float-start'>Loan ID: <span className='orangeText'>{loanId}</span></div>
                                ...
                                <button type="submit" onClick={() => { LoanhandleShow(); handleClose() }} className="btn btn-primary addBtn float-end me-0"><i class="fa-solid fa-print"></i> Preclose Loan</button>
                        ...
                        <Modal.Body>
                            {loadingView ? <Loading /> :
                                <DataTable fixedHeader fixedHeaderScrollHeight="70vh" className='girdTable' columns={EmiTableColumns} data={loanCollList} />
                            }
                        </Modal.Body>
                        <Modal.Footer>
            ...
            <div >
                    ...
                    <Modal size="lg" show={approveshow} onHide={handleClose2}>
                        ...
                        <Modal.Header>
                            ...
                            <div className="form-group StatusCheckbox">
                                ...
                                <input type="checkbox" checked={approve == "yes" ? true : false} value={approve == "no" ? "yes" : "no"} disabled={loginDetails[0].user_type === "Super Admin" /* && status === 1 */ ? false : true} onChange={(e) => { getEditLoanDeatils(e.target.value, "Approve"); setApprove(e.target.value) }} />
                        ...
                        <Modal.Body>
                            ...
                            <div className="row loansBlock">
                                ...
                                <div className='col-lg-12 mb-2'>
                                    ...
                                    <Radio inputId="daily" labelText="Daily" radioChecked={loanType == "daily" ? true : false} onChange={(e) => { getEditLoanDeatils(e.target.value, "daily"); pclear() }} />
                                    <Radio inputId="weekly" labelText="Weekly" radioChecked={loanType == "weekly" ? true : false} onChange={(e) => { getEditLoanDeatils(e.target.value, "weekly"); pclear() }} />
                                    <Radio inputId="monthly" labelText="Monthly" radioChecked={loanType == "monthly" ? true : false} onChange={(e) => { getEditLoanDeatils(e.target.value, "monthly"); pclear() }} />
                                </div>
                                {/* daily fields */}
                                {loanType == "daily" ?
                ...
                <Modal size="md" show={LoanClose} onHide={LoanhandleClose2}>

                    <Modal.Header>
                        <Modal.Title>
                            <h2>Loan Closed Details</h2>
                            <div className='loanAmt'>
                                <div className='float-start'>Loan ID: <span className='orangeText'>{loanId}</span></div>
                                <div className='float-start'>Loan Amount: <span> {amountShow}/{totalAmount} </span> </div>
                            </div>
                    ...
                    <Modal.Body>
                        ...
                        <div >
                            ...
                            <input className={`${!principalcloaseAmount && (submitClicked) ? 'error' : ''}`} type="number" placeholder="Principal amount" name="principalcloaseAmount" onChange={(e) => setPrincipalcloseamount(e.target.value)} onKeyDown={e => exceptThisSymbols.includes(e.key) && e.preventDefault()} value={principalcloaseAmount} />

                        </div><br />
                        <div >
                            <label>Interest Amount:</label>

                            <input className={`${!interestcloaseAmount && (submitClicked) ? 'error' : ''}`} type="number" placeholder="Interest amount" name="interestcloaseAmount" onChange={(e) => setInterestcloseamount(e.target.value)} onKeyDown={e => exceptThisSymbols.includes(e.key) && e.preventDefault()} value={interestcloaseAmount} />
                    ...
                    <Modal.Footer>
                        ...
                        <div className="col-lg-12 text-center">
                            ...
                            <button class="btn btn-primary smallBtn" onClick={closeLoan}>Save</button>
                            <button className="btn btn-primary btn-outline smallBtn" onClick={LoanhandleClose2}>
                                Close
                            </button>
                ...
                <Modal size="sm" show={closecnfrmShow} onHide={handleClose5}>
                    ...
                    <Modal.Body>
                        Are you sure do you want to Close the Loan
                    </Modal.Body>
                    <Modal.Footer>



                        <button className="btn btn-primary btn-outline smallBtn" onClick={closeLoan}>
                            Save
                        </button>
                        <button className="btn btn-primary btn-outline smallBtn" onClick={handleClose5}>
                            Close
                        </button>
                    </Modal.Footer>
            ...
            <div class="hideContent">
                <div id="Print1" onClick={document.title = "MonthlyFinance"} style={{ "border-collapse": "collapse" }} >
                    {/* <h1>MonthlyFinance</h1> */}
            ...
            <div class="hideContent">
                <div id="Print" onClick={document.title = "MonthlyFinance"} style={{ "border-collapse": "collapse" }} >
                    <div className="card-body">
                        <div className="row loansBlock">
                            <div class="col-lg-6">
                                ...
                                <div class="col-lg-12">
                                    <p className="form-label"> Remaining Months </p>
                                ...
                                <div class="col-lg-12">
                                    <p className="form-label"> Document Charge </p>
                                    <p className='form-value'>:&nbsp;{header.interest_amount}</p>
                                </div>
                                <div class="col-lg-12">
                                    <p className="form-label"> Interest </p>
                                    <p className='form-value'>:&nbsp;{header.interest}</p>
                                </div>
                                {/* start_date */}
                                <div class="col-lg-12">
                                    <p className="form-label"> Start Date </p>

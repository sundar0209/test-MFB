...
const WeeklyLoanFinance = () => {
  ...
  const [loadingLoan, setLoadingLoan] = useState(true);
    ...
    const [submitWeeklyLoanFinance, setsubmitWeeklyLoanFinance] = useState(false)
    ...
    const checkClear = () => {
        setReferencename("");
        setReferencephoneno("");
        setsubmitWeeklyLoanFinance(false);
    }
    console.log(list, "numberarray list");
    ...
    //const calculationn = Math.round((Number(loanAmount)) / Number(noOfDays))
    ...
    const singleLoan = (data) => {
        ...
        axiosObject.post("loanview/condition", request)
            ...
            .then((response) => {
                ...
                if (response.data.data[0]?.reference_person === "yes") {
    ...
    const multiListDetails = () => {
        ...
        axiosObject.post("arealistdropdown/condition", request).then(response => {
            ...
            setCustInfo(response.data.data.map((data) => { return { "label": data.area, "value": data.area } }));
    ...
    const GroupList = (data) => {
        console.log("numberarray data", data.map((data) => data.label));

        const areaVal = data.map((data) => "'" + data.label + "'")
        ...
        console.log("area", request);
        axiosObject.post("weeklyAreaFilter/condition", request).then(response => {
            setDailyFinList(response.data.data);
            setLoading(false);
        });
    }
    ...
    const editLoanapprove = () => {
        ...
        if (Enablereference === "yes") {
            setsubmitWeeklyLoanFinance(true);
            if (!referencename) {
                console.log("");
        ...
        let request = {
            ...
            history_description: editLoanVal.slice(0, -1),
            ...
            axiosObject.post("custloanApprove/update", request)
                .then((response) => {
                    ...
                    if (response.data.success == true) {
                        ...
                        setPopupActionPath("/weeklyloanfinance")
                        handleClose()
                    }
                    else {
                        togglePopup()
    ...
    // const totalAmount = Math.round(header.totalpaidamount)
            ...
            cell: (row, index) => {
                ...
                return (
                    ...
                    <div>
                        ...
                        <input className='form-control' type="number" placeholder="Penalty Fees" id={`inputPenaltyId${index}`} name="Penalty" onChange={(e) => { handleChangeField(e.target.value, row, "Penalty/Discount"); penalyDiscount(); }} onKeyDown={e => exceptThisSymbols.includes(e.key) && e.preventDefault()} defaultValue={row.penalty_discount} />
            ...
            cell: (row, index) => {
                ...
                return (
                    ...
                    <div className='commentsDiv'>
                        <textarea className='form-control' type="text" placeholder="comments" id={`comments${index}`} name="comments" onChange={(e) => { getLoanDeatilspopup(e.target.value, "Comments"); setComments(e.target.value); handleChangeField(e.target.value, row, "Comments") }} defaultValue={row.comments == "null" ? "" : row.comments} />
            ...
            cell: (row, index, ispaid) => {
                ...
                return (
                    <div className='tableBtns emiTableActions'>
                        ...
                        {/* <StatusCheckbox className="smsenable" labelText="" inputId={`checkboxId${index}`} onClick={(e) => { handleChangeField(e.target.value, row, "CheckBox"); printCheck(`checkboxId${index}`, `editEnableId${index}`, `inputEnableId${index}`, `inputAmountId${index}`, `inputPenaltyId${index}`, `comments${index}`, row) }} /> */}
                        {/* {ispaid == "No" ?
                            ...
                            : <button id={`editEnableId${index}`} className="btn btn-primary btn-outline smallBtn" onClick={() => { enableDate(`checkboxId${index}`, `editEnableId${index}`, `inputEnableId${index}`, `inputAmountId${index}`, `inputPenaltyId${index}`, `comments${index}`, row) }}>
                                <i class="fa-solid fa-pencil"></i>
                            </button>} */}
                        <StatusCheckbox className="smsenable" labelText="" inputId={`checkboxId${index}`} onClick={() => handleChangeField(`checkboxId${index}`, row, "CheckBox")} />
        ...
        if (label === "No of Days") {
            ...
            if (item !== "") {
                if (editloanLables.length > 0) {
                    if (editloanLables.filter((data) => data.label === label).length === 0) {
                        // console.log("if calling");
                        setEditloanLables([...editloanLables, { label }, { "label": "Due Amount Per Day" }]);
                    }
                }
                else {
                    // console.log("1st else calling");
                ...
                // console.log("else calling");
                setEditloanLables(editloanLables.filter((data) => data?.label !== label && data?.label !== "Due Amount Per Day"))
            }
        }
        ...
        if (label === "No of Months") {
            ...
            if (item !== "") {
                if (editloanLables.length > 0) {
                    if (editloanLables.filter((data) => data.label === label).length === 0) {
                        // console.log("if calling");
                        setEditloanLables([...editloanLables, { label }, { "label": "Due Amount Per Months" }]);
                    }
                }
                else {
                    // console.log("1st else calling");
                ...
                // console.log("else calling");
                setEditloanLables(editloanLables.filter((data) => data?.label !== label && data?.label !== "Due Amount Per Months"))
            }
        }
                    ...
                    {loading ? <Loading /> :
                        <div className="row">
                            ...
                            <div className="col-lg-8 col-md-8 col-sm-8 col-8 tableSearchBlock">
                                ...
                                <div className='col-4 selectStatus'>
                                    <div className="form-group">
                                        <select className="form-select" id="Loanstatus" onChange={(e) => dailyloanFilter(e.target.value)}>
                    ...
                    <Modal.Body>
                        ...
                        <div className="row loansBlock">
                            ...
                            <div className='col-lg-12 mb-2'>
                                ...
                                <Radio className={`${!numberOfWeeks && (submitweeklyLoanClicked) ? 'error' : ''}`} inputId="6_Weeks" inputName="6 Weeks" labelText="6 Weeks" radioChecked={numberOfWeeks === "6" || numberOfWeeks === 6 ? true : false} onChange={() => { handleSelectWeeklys("6"); checkboxfieldclear(); handleError(); checkClear() }} />
                                <Radio className={`${!numberOfWeeks && (submitweeklyLoanClicked) ? 'error' : ''}`} inputId="12_Weeks" inputName="6 Weeks" labelText="12 Weeks" radioChecked={numberOfWeeks === "12" || numberOfWeeks === 12 ? true : false} onChange={() => { handleSelectWeeklys("12"); checkboxfieldclear(); handleError(); checkClear() }} />
                            </div>
                            ...
                            <div class="row">
                                ...
                                {/* <div className="col-lg-4 col-md-6">
                                    ...
                                    <InputText className={`${!referencename && (submitClicked) ? 'error' : ''}`} onChange={(e) => { setReferencename(e.target.value) }} id="RefinputId" labelText="ReferenceName" inputValue={referencename} inputName="Referencename" inputType="text" inputDisable={Enablesmsedit == "no"} />

                                </div>
                                <div className="col-lg-4 col-md-6">

                                    <InputText className={`${!referencephoneno && (submitClicked) ? 'error' : ''}`} onChange={(e) => { getEditLoanDeatils(e.target.value, "ReferencePhoneno"); setReferencephoneno(e.target.value.replace(/[^0-9]/g, "")) }} maxLength={10} labelText="ReferencePhoneno" inputValue={referencephoneno} inputName="ReferencePhoneno" inputType="text" inputDisable={Enablesmsedit == "no"} />

                                </div> */}
                                <div className="col-12 loanHead">
                                    <input className={`${!Enablereference && (submitWeeklyLoanFinance) ? 'error' : ''}`} type="checkbox" label="Reference Person" checked={Enablereference == "yes" ? true : false} value={Enablereference == "no" ? "yes" : "no"} onChange={(e) => { setEnablereference(e.target.value); setNumber1(true); checkClear() }} id="reference" />
                                    <label className="form-label"> Reference Person</label>
                                </div>
                                {Enablereference === "yes" &&
                                    <>
                                        <div className="col-lg-4 col-md-6">
                                            <InputText className={`${!referencename && (submitWeeklyLoanFinance) ? 'error' : ''}`} onChange={(e) => { getEditLoanDeatils(e.target.value, "ReferenceName") }} id="RefinputId" labelText="ReferenceName" inputValue={referencename} inputName="Referencename" inputType="text" inputDisable={Enablereference == "no"} />

                                        </div>
                                        <div className="col-lg-4 col-md-6">
                                            <InputText className={`${!referencephoneno && (submitWeeklyLoanFinance) ? 'error' : ''}`} onChange={(e) => { getEditLoanDeatils(e.target.value, "ReferencePhoneno"); setReferencephoneno(e.target.value.replace(/[^0-9]/g, "")) }} maxLength={10} labelText="ReferencePhoneno" inputValue={referencephoneno} inputName="ReferencePhoneno" inputType="text" inputDisable={Enablereference == "no"} />
                    ...
                    <table style={{ width: "100%" }}>
                        <thead>
                            <tr>
                                ...
                                <th class="border-top-0">LoanType</th>
                            ...
                            {dailyFinList?.length > 0 ? (
                                dailyFinList.map((dailyFinList) => (
                                    <tr>
                                        ...
                                        <td>{dailyFinList.weeklyinstallmentnumber}</td>
                                        <td>{dailyFinList.loan_amount}</td>
                                        <td>{dailyFinList.reference_person_name}</td>
                                        <td>{dailyFinList.reference_person_phoneno}</td>
...
export default WeeklyLoanFinance;